...
### 第三阶段：SaaS 服务化 (V1.3)
重点在于多用户支持与在线服务部署。
- **飞书 OAuth 登录：** 支持用户通过飞书扫码登录，自动创建账户。
- **用户鉴权体系：** 基于 `user_access_token` 的权限管理，实现“用户 -> 自己的飞书云文档”的数据隔离。
- **在线部署：** 支持 Docker 部署，提供公开服务能力。
- **BYOK 模式 (可选)：** 允许高级用户配置自己的 Feishu App ID/Secret 以绕过公共频率限制。
